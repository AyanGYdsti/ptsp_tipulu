document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateCarousel() {
        const itemWidth = carouselInner.children[0].getBoundingClientRect().width;
    ...
    const countUpObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                document.querySelectorAll('[data-target]').forEach(animateCountUp);
        ...
        while (retries > 0) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.warn(`API call throttled. Retrying in ${delay / 1000}s...`);
    ...
    const addMessageToChat = (sender, message, isLoading = false) => {
        ...
        messageBubble.className = `max-w-xs md:max-w-sm rounded-2xl px-4 py-2.5 ${sender === 'user' ? 'bg-blue-600 text-white rounded-br-lg' : 'bg-gray-200 text-gray-800 rounded-bl-lg'}`;
    ...
    addMessageToChat('bot', 'Halo! Ada yang bisa saya bantu terkait layanan di Kelurahan Tipulu?');
    ...
    geminiAssistBtn.addEventListener('click', async () => {
        ...
        if (!userText) {
            geminiStatus.innerText = 'Mohon isi detail pengaduan terlebih dahulu.';
            return;
        }
        
        geminiAssistBtn.disabled = true;
        geminiStatus.innerHTML = '<div class="flex justify-center items-center gap-2"><div class="loader"></div><span>Memproses tulisan Anda...</span></div>';
        ...
        setTimeout(() => { geminiStatus.innerText = ''; }, 4000);
    });
});